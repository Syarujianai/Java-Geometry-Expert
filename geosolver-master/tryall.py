...
match_chars = []
for i in range(len(os.listdir('tmp/all_images/')[:100])):   # len(os.listdir('tmp/all_images/')))
    if i % 20 == 0:
        print(i)
    match_char = {}
    try:
        ...
        print('fail"', i)
    match_chars.append(match_char)
    print(match_char)
# with open('match_char.pk', 'wb') as f:
#     pickle.dump(match_chars, f)
